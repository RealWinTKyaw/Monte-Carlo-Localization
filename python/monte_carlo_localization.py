...
class ParticleFilter(object):
    def __init__(self, num_particles, occ_grid_map, xmin, xmax, ymin, ymax,
                 ...
                 beam_range_measurement_noise_std_dev, n_thresh=0):
    ...
    def exponentiate_and_normalize(x):
        #print("I am in exponentiate_and_normalize")
        b = x.max()
        x = np.exp(x - b)
        return x / x.sum()
    ...
    def metric_to_grid_coords(self, x, y): 
        ...
        row = min(max(int(gy), 0), height)
        col = min(max(int(gx), 0), width)
        return (row, col)
...
class MonteCarloLocalization(Node):
    ...
    def odometry_callback(self, msg):
        ...
        self.mutex.release()
        #print(" I am in odometry_callback")
    ...
    def get_particle_marker(self, particle, marker_id):
        ...
        particle_marker.color = ColorRGBA(r=0.0,g=1.0,b=0.0,a=1.0)
        ...
        print("I am in get_particle_marker")
    ...
    def publish_particle_markers(self):
        ...
        for i, particle in enumerate(self.pf.particles):
            print("I am  in publish_particle_markers")
